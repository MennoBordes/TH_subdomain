...
namespace TH.Core.Base.Extensions
{
    public static class StringBuilderExtensions
    {
        ...
        {
            return sb.ToString(startIndex, sb.Length - 1);
        }
        ...
        /// <summary> Get the last index of a string. </summary>
        ...
        /// <summary> Finds all occurences between the 2 specified values. </summary>
        public static List<string> FindAllOccurences(this StringBuilder sb, string start, string end)
        {
            List<string> result = new List<string>();

            if (start.Length > sb.Length)
                return result;
            if (end.Length > sb.Length)
                return result;
            ...
            {
                string startResult = i + start.Length > sb.Length ? null : sb.Substring(i, start.Length);
                string endResult = i + end.Length > sb.Length ? null : sb.Substring(i, end.Length);

                if (!startFound && startResult == start)
                {
                    startFound = true;
                    occurence.Append(startResult);
                    startEnd = i + start.Length;
                }
                else if (!endFound && endResult == end)
                {
                    endFound = true;
                    occurence.Append(endResult);

                    // Reset and add to list
                ...
                else if (startFound && !endFound && i >= startEnd)
                {
                    occurence.Append(sb[i]);
                }
            }
        ...
        /// <summary> Checks if the stringbuilder is null or empty; </summary>
